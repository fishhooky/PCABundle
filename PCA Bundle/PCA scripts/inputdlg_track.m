function Answer=inputdlg_track(Prompt, Title, NumLines, DefAns, Resize)
...
%set(InputFig,'units','normalized'); % why doesnt this work?
%set(InputFig,'Position',[0.3 0.1 0.04 0.08]);

resolution = get(0,'screensize'); % get the screen resolution size to determine the center in pixels
set(InputFig,'Position',[((resolution(3)/2)-125) ((resolution(4)/2)-250) 250 500]);%getnicedialoglocation(FigPos,get(InputFig,'Units')));

OKHandle=uicontrol(InputFig     ,              ...
  BtnInfo      , ...
  'Position'   ,[ FigWidth-2*BtnWidth-8*DefOffset DefOffset BtnWidth+20 BtnHeight ] , ...
