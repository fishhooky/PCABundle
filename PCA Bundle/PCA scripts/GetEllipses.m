function [Coordinates,logicTEST,CF]=GetEllipses(data1,replicates,CF,test,test_replicates)
...
% data1 = training set data. Note only first two columns used.
...
% logicTEST will output 1 for every test datapoint that is within the
% confidence limit and 0 if outside. Different row for each sample set.
% Confidence limit set by CF
...
for m=1:Loops
    ...
    [r_ellipse,lTEST,CF]=error_ellipse(data,test1,CF);    
